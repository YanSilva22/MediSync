...
import time
from SQLfunctions import DBConnection
import threading
import sys
...
class Application(object):
    ...
    def logSQL(self):
        ...
        self.loginSql.title("CONFIGURAR SQL")
        self.loginSql["bg"] = "#26a599"
        self.loginSql.maxsize(width=self.loginSql.winfo_screenwidth(),
                              height=self.loginSql.winfo_screenheight())
        self.loginSql.minsize(width=700, height=600)
        self.loginSql.resizable(False, False)
        # centralizando a janela
        # dimensões
        largura = 700
        altura = 600
        # resolução
        largura_screen = self.loginSql.winfo_screenwidth()
        altura_screen = self.loginSql.winfo_screenheight()
        # posição
        posx = largura_screen/2 - largura/2
        posy = altura_screen/2 - altura/2
        # geometry
        self.loginSql.geometry('%dx%d+%d+%d' %
        ...
        # explicação pagina
        # label espec
        self.quadrado = tk.Label(self.loginSql, text="FAÇA O LOGIN NO SQL PARA \nACESSAR AS DEMAIS FUNCIONALIDADES", bg="#22958a", fg='white',
                                 font=('Comic Sans MS', 18, "bold"), anchor="center")

        # host
        # Label host
        self.host = tk.Label(self.loginSql, text="HOST", bg='#26a599',
                             fg='white', anchor='center', font=('Comic Sans MS', 16, "bold"))
        # Entry user
        self.entryHost = tk.Entry(self.loginSql, fg='white', bg='#22958a',
                                  font=('Comic Sans MS', 14, "bold"))

        # User
        # Label USer
        self.user = tk.Label(self.loginSql, text="USER", bg='#26a599',
                             fg='white', anchor='center', font=('Comic Sans MS', 16, "bold"))
        # Entry user
        self.entryUser = tk.Entry(self.loginSql, fg='white', bg='#22958a',
                                  font=('Comic Sans MS', 14, "bold"))

        # senha
        # Label senha
        self.senha = tk.Label(self.loginSql, text="SENHA", bg='#26a599',
                              fg='white', anchor='center', font=('Comic Sans MS', 16, "bold"))
        # Entry SENHA
        self.entrySenha = tk.Entry(self.loginSql, fg='white', bg='#22958a',
                                   font=('Comic Sans MS', 14, "bold"))
        ...
        # atribuindo comando
        self.save = tk.Button(self.loginSql, image=btnsalvar, bg='#26a599', fg='white', bd=0,
                              command=lambda: self.validarLogin(self.entryHost, self.entryUser, self.entrySenha))

        ### mudando a cor do botão ao passar cursor###
        self.save.bind("<Enter>", lambda e1,
                       b1=self.save: self.on_enter(e1, b1))
        self.save.bind("<Leave>", lambda e1,
                       b1=self.save: self.on_leave(e1, b1))

        ####################################################
        ######### 2 - EXIBINDO OS COMPONENTES ##############
        ##### label destaque reologio e nome do app#####
        self.quadrado.place(relx=0.005, rely=0.01,
                            relwidth=0.989, relheight=0.1)
        self.host.place(relx=0.05, rely=0.25125,
                        relwidth=0.40, relheight=0.075)
        self.user.place(relx=0.05, rely=0.4675, relwidth=0.40, relheight=0.075)
        self.senha.place(relx=0.05, rely=0.68375,
                         relwidth=0.40, relheight=0.075)
        ######## geometry  botões ########
        self.save.place(relx=0.375, rely=0.8875,
                        relwidth=0.25, relheight=0.1)
        ######## geometry entry ########
        self.entryHost.place(relx=0.5, rely=0.25125,
                             relwidth=0.40, relheight=0.075)
        self.entryUser.place(relx=0.5, rely=0.4675,
                             relwidth=0.40, relheight=0.075)
        self.entrySenha.place(relx=0.5, rely=0.68375,
                              relwidth=0.40, relheight=0.075)
        ######## focus ########
        self.entryHost.focus_force()
        self.loginSql.protocol(
            "WM_DELETE_WINDOW")

    def validarLogin(self, host, user, senha):
        if (host.get() == "" or user.get() == "" or senha.get() == ""):
            self.app.msgbox = messagebox.showerror(
                title="Error", message="POR FAVOR PREENCHA TODOS OS DADOS!")
        else:
            self.currentSession = DBConnection(host.get(), user.get(), senha.get())
            quickCheck = self.currentSession.conectar_bd()
            if (quickCheck == None):
                self.app.msgbox = messagebox.showerror(title= "Error", message="Erro ao conectar ao banco de dados!\nInformações invalidas.")
            else:
                self.app.msgbox = messagebox.showinfo(
                    title="Result", message= "Dados salvos com sucesso!")
                self.mudarJanela(self.app, self.loginSql)
    ...
    def widgets(self):
        ...
        ####### CADASTRO PACIENTE #######
        # Label paciente
        self.desc01 = tk.Label(self.app, text="CADASTRAR PACIENTE!", bg='#e1fada',
        ...
        ####### CADASTRO CONSULTA #######
        # Label Consulta
        self.desc02 = tk.Label(self.app, text="CADASTRAR CONSULTA!", bg='#e1fada',
                               fg='#26a599', anchor='center', font=('Comic Sans MS', 14, "bold"))
        # Imagem consulta
        ...
        # Label funcionario
        self.desc03 = tk.Label(self.app, text="CADASTRAR FUNCIONARIO! ", bg='#e1fada',
                               fg='#26a599', anchor='center', font=('Comic Sans MS', 14, "bold"))
        # Imagem Funcionario
    ...
    def frameVisPaciente(self):
        ...
        self.labelTv.column('dataNascimento', minwidth=0,
        ...
        self.labelTv.heading(
            'dataNascimento', text='DATA DE NASCIMENTO', anchor="center")
        self.labelTv.pack(fill="both", expand=True)

        pacientes = self.currentSession.visualizar_pacientes()
        if pacientes:
        ...
        self.quadrado.place(relx=0.005, rely=0.01,
                            relwidth=0.989, relheight=0.1)
        self.labelTv.place(relx=0.05, rely=0.1585,
                           relwidth=0.89, relheight=0.6664)
        ######## geometry  botões ########
        self.deletar.place(relx=0.1666, rely=0.8875,
                           relwidth=0.25, relheight=0.1)
        self.voltar.place(relx=0.5766, rely=0.8875,
                          relwidth=0.25, relheight=0.1)
    ...
    def frameVisConsulta(self):
        ...
        # adicionando o tamanho das colunas
        self.labelTv.column('id', minwidth=0, width=30, anchor="center")
        self.labelTv.column('nomeDoutor', minwidth=0,
                            width=60, anchor="center")
        self.labelTv.column('nomePaciente', minwidth=0,
                            width=60, anchor="center")
        self.labelTv.column('valorConsulta', minwidth=0,
                            width=70, anchor="center")
        self.labelTv.column('descricao', minwidth=0, width=50, anchor="center")
        self.labelTv.column('dataConsulta', minwidth=0,
                            width=50, anchor="center")
        self.labelTv.column('horaConsulta', minwidth=0,
        ...
        self.labelTv.heading(
            'nomePaciente', text='NOME PACIENTE', anchor="center")
        self.labelTv.heading(
            'valorConsulta', text='VALOR DA CONSULTA', anchor="center")
        self.labelTv.heading('descricao', text='DESCRIÇÂO', anchor="center")
        self.labelTv.heading(
            'dataConsulta', text='DATA DA CONSULTA', anchor="center")
        self.labelTv.heading(
            'horaConsulta', text='HORA DA CONSULTA', anchor="center")

        consultas = self.currentSession.visualizar_agenda()
        if consultas:
        ...
        self.quadrado.place(relx=0.005, rely=0.01,
                            relwidth=0.989, relheight=0.1)
        self.labelTv.place(relx=0.025, rely=0.1585,
                           relwidth=0.95, relheight=0.6664)
        ######## geometry  botões ########
        self.deletar.place(relx=0.1666, rely=0.8875,
                           relwidth=0.25, relheight=0.1)
    ...
    def frameVisFuncionario(self):
        ...
        self.labelTv.heading(
            'nomeDoutor', text='NOME FUNCIONARIO', anchor="center")
        ...
        funcionarios = self.currentSession.visualizar_medicos()
        if funcionarios:
    ...
    def janelaPaciente(self):
        ...
        self.quadrado.place(relx=0.005, rely=0.01,
                            relwidth=0.989, relheight=0.1)
        self.nome.place(relx=0.05, rely=0.17916,
                        relwidth=0.40, relheight=0.075)
        self.email.place(relx=0.05, rely=0.32332,
                         relwidth=0.40, relheight=0.075)
        self.telefone.place(relx=0.05, rely=0.467486,
                            relwidth=0.40, relheight=0.075)
        self.cpf.place(relx=0.05, rely=0.61646, relwidth=0.40, relheight=0.075)
        self.dNascimento.place(relx=0.03, rely=0.755806,
                               relwidth=0.5, relheight=0.075)
        ######## geometry  botões ########
        self.save.place(relx=0.05, rely=0.8875, relwidth=0.25, relheight=0.1)
        self.vizualizacao.place(relx=0.375, rely=0.8875,
                                relwidth=0.25, relheight=0.1)
        self.voltar.place(relx=0.7, rely=0.8875, relwidth=0.25, relheight=0.1)

        ######## geometry entry ########
        self.entryNome.place(relx=0.5, rely=0.17916,
                             relwidth=0.40, relheight=0.075)
        self.entryEmail.place(relx=0.5, rely=0.32332,
                              relwidth=0.40, relheight=0.075)
        self.entryTelefone.place(
            relx=0.5, rely=0.467486, relwidth=0.40, relheight=0.075)
        self.entryCpf.place(relx=0.5, rely=0.61646,
                            relwidth=0.40, relheight=0.075)
        self.sbxDia.place(relx=0.52, rely=0.755806,
                          relwidth=0.13, relheight=0.075)
        self.sbxMes.place(relx=0.663, rely=0.755806,
                          relwidth=0.133, relheight=0.075)
        self.sbxAno.place(relx=0.8063, rely=0.755806,
                          relwidth=0.13, relheight=0.075)
        ######## focus ########
    ...
    def validarFormsPaciente(self, nome, email, telefone, cpf, dia, mes, ano):
        if (nome == "" or email == "" or telefone == "" or cpf == "" or dia == "" or mes == "" or ano == ""):
            ...
            check, message = self.currentSession.adicionar_paciente(
                nome, dtNascimento, telefone, email, cpf)
            if check:
                self.app.msgbox = messagebox.showinfo(
                ...
                self.app.msgbox = messagebox.showerror(
                    title="Deu ruim", message=message)
                self.mudarJanela(self.app, self.frameFuncionario)
    ...
    def deletar_pacientes(self, table):
        ...
        if selected_item:
            ...
            checkOpc = messagebox.askokcancel(
                "Comfirmação", "Deseja deletar o item selecionado?")
            deleter, msg = self.currentSession.removedor_pacientes(
                id_item, checkOpc)
            if deleter:
    ...
    def janelaConsulta(self):
        ...
        self.quadrado.place(relx=0.005, rely=0.01,
                            relwidth=0.989, relheight=0.1)
        self.idMedico.place(relx=0.05, rely=0.1585,
                            relwidth=0.40, relheight=0.075)
        self.idPaciente.place(relx=0.05, rely=0.2821,
                              relwidth=0.40, relheight=0.075)
        self.valorconsulta.place(
            relx=0.05, rely=0.4057, relwidth=0.40, relheight=0.075)
        self.descrição.place(relx=0.05, rely=0.5292,
                             relwidth=0.40, relheight=0.075)
        self.dataConsulta.place(relx=0.05, rely=0.6528,
                                relwidth=0.40, relheight=0.075)
        self.Horario.place(relx=0.05, rely=0.7764,
                           relwidth=0.40, relheight=0.075)
        ######## geometry  botões ########
        self.save.place(relx=0.05, rely=0.8875, relwidth=0.25, relheight=0.1)
        self.vizualizacao.place(relx=0.375, rely=0.8875,
        ...
        self.sbxMinuto.place(relx=0.74, rely=0.7764,
                             relwidth=0.133, relheight=0.075)
        ######## focus ########
        self.entryIdMedico.focus_force()
    ...
    def validarFormsConsulta(self, idDoutor, idPaciente, valConsulta, dia, mes, ano, hora, minuto, desc):
        if (idDoutor == "" or idPaciente == "" or valConsulta == ""
            ...
            check, message = self.currentSession.agendar_consulta(
                idPaciente, idDoutor, dtConsulta, horaConsulta, desc, valConsulta)
            if check:
                self.app.msgbox = messagebox.showinfo(
                    title="Result", message=message)
    ...
    def deletar_consultas(self, table):
        ...
        if selected_item:
            ...
            checkOpc = messagebox.askokcancel(
                "Comfirmação", "Deseja deletar o item selecionado?")
            deleter, msg = self.currentSession.removedor_consultas(
                id_item, checkOpc)
            if deleter:
                messagebox.showinfo(title="Sucesso", message=msg)
            else:
    ...
    def janelaFuncionario(self):
        ...
        self.quadrado.place(relx=0.005, rely=0.01,
                            relwidth=0.989, relheight=0.1)
        self.nomeMedico.place(relx=0.05, rely=0.15857,
                              relwidth=0.40, relheight=0.075)
        self.emailMed.place(relx=0.05, rely=0.28214,
                            relwidth=0.40, relheight=0.075)
        self.telefoneMed.place(relx=0.05, rely=0.4057,
                               relwidth=0.40, relheight=0.075)
        self.crm.place(relx=0.05, rely=0.5292, relwidth=0.40, relheight=0.075)
        self.dNascimentoMed.place(
            relx=0.03, rely=0.65285, relwidth=0.5, relheight=0.075)
        self.cpfMed.place(relx=0.05, rely=0.7764,
                          relwidth=0.4, relheight=0.075)
        ...
        ######## geometry entry ########
        self.entryNomeMedico.place(
            relx=0.5, rely=0.15857, relwidth=0.40, relheight=0.075)
        self.entryEmailMed.place(
            relx=0.5, rely=0.28214, relwidth=0.40, relheight=0.075)
        self.entryTelefoneMed.place(
            relx=0.5, rely=0.4057, relwidth=0.40, relheight=0.075)
        self.entryCrm.place(relx=0.5, rely=0.5292,
                            relwidth=0.40, relheight=0.075)
        self.sbxDiaMed.place(relx=0.52, rely=0.65285,
                             relwidth=0.13, relheight=0.075)
        self.sbxMesMed.place(relx=0.663, rely=0.65285,
                             relwidth=0.133, relheight=0.075)
        self.sbxAnoMed.place(relx=0.8063, rely=0.65285,
                             relwidth=0.13, relheight=0.075)
        self.entryCpfMed.place(relx=0.5, rely=0.7764,
                               relwidth=0.40, relheight=0.075)
        ######## focus ########
    ...
    def validarFormsFuncionario(self, nomeDoutor, email, telefone, crm, dia, mes, ano, cpf):
        if (nomeDoutor == "" or email == "" or telefone == ""
            ...
            self.data.append(data)"""
            dtNascimento = ano+"-"+mes+"-"+dia
            check, message = self.currentSession.adicionar_medico(
                nomeDoutor, email, telefone, crm, dtNascimento, cpf)
            if check:
                self.app.msgbox = messagebox.showinfo(
                    title="Result", message=message)
                self.mudarJanela(self.app, self.frameFuncionario)
    ...
    def deletar_funcionarios(self, table):
        ...
        if selected_item:
            ...
            checkOpc = messagebox.askokcancel(
                "Comfirmação", "Deseja deletar o item selecionado?")
            deleter, msg = self.currentSession.removedor_medicos(id_item, checkOpc)
            if deleter:
                messagebox.showinfo(title="Sucesso", message=msg)
            else:
                messagebox.showinfo(title="Error", message=msg)
